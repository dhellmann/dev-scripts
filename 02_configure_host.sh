...
if [ "$MANAGE_PRO_BRIDGE" == "y" ]; then
    ...
    if [ ! -e /etc/sysconfig/network-scripts/ifcfg-${PRO_BRIDGE_NAME} ] ; then
        echo -e "DEVICE=${PRO_BRIDGE_NAME}\nTYPE=Bridge\nONBOOT=yes\nNM_CONTROLLED=no\nBOOTPROTO=static\nIPADDR=172.22.0.1\nNETMASK=255.255.255.0${ZONE}" | sudo dd of=/etc/sysconfig/network-scripts/ifcfg-${PRO_BRIDGE_NAME}
    fi
...
if [ "$MANAGE_INT_BRIDGE" == "y" ]; then
    ...
    if [ ! -e /etc/sysconfig/network-scripts/ifcfg-${INT_BRIDGE_NAME} ] ; then
        echo -e "DEVICE=${INT_BRIDGE_NAME}\nTYPE=Bridge\nONBOOT=yes\nNM_CONTROLLED=no${ZONE}" | sudo dd of=/etc/sysconfig/network-scripts/ifcfg-${INT_BRIDGE_NAME}
    fi
    ...
    if [ "$INT_IF" ]; then
        ...
        if sudo nmap --script broadcast-dhcp-discover -e $INT_IF | grep "IP Offered" ; then
            grep -q BOOTPROTO /etc/sysconfig/network-scripts/ifcfg-${INT_BRIDGE_NAME} || (echo -e "\nBOOTPROTO=dhcp\n" | sudo tee -a /etc/sysconfig/network-scripts/ifcfg-${INT_BRIDGE_NAME})
        fi
...
for PORT in 80 5000 ; do
    if [ "${RHEL8}" = "True" ] ; then
        ...
        if ! sudo iptables -C INPUT -i ${PRO_BRIDGE_NAME} -p tcp -m tcp --dport $PORT -j ACCEPT > /dev/null 2>&1; then
            sudo iptables -I INPUT -i ${PRO_BRIDGE_NAME} -p tcp -m tcp --dport $PORT -j ACCEPT
        fi
        if ! sudo iptables -C INPUT -i ${INT_BRIDGE_NAME} -p tcp -m tcp --dport $PORT -j ACCEPT > /dev/null 2>&1; then
            sudo iptables -I INPUT -i ${INT_BRIDGE_NAME} -p tcp -m tcp --dport $PORT -j ACCEPT
        fi
    fi
done
...
if [ "${RHEL8}" = "True" ] ; then
    ...
    if ! sudo iptables -C INPUT -i ${INT_BRIDGE_NAME} -p udp -m udp --dport 6230:${VBMC_MAX_PORT} -j ACCEPT 2>/dev/null ; then
        sudo iptables -I INPUT -i ${INT_BRIDGE_NAME} -p udp -m udp --dport 6230:${VBMC_MAX_PORT} -j ACCEPT
    fi
fi
