...
if [ "${RHEL8}" = "True" ] ; then
    sudo firewall-cmd --zone=libvirt --add-port=${VMBC_BASE_PORT}-${VBMC_MAX_PORT}/udp
    sudo firewall-cmd --zone=libvirt --add-port=${VMBC_BASE_PORT}-${VBMC_MAX_PORT}/udp --permanent
else
    if ! sudo $IPTABLES -C INPUT -i ${BAREMETAL_NETWORK_NAME} -p udp -m udp --dport ${VMBC_BASE_PORT}:${VBMC_MAX_PORT} -j ACCEPT 2>/dev/null ; then
        sudo $IPTABLES -I INPUT -i ${BAREMETAL_NETWORK_NAME} -p udp -m udp --dport ${VMBC_BASE_PORT}:${VBMC_MAX_PORT} -j ACCEPT
    fi
fi
